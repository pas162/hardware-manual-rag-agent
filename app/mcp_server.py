...
# ── Figure handoff for Smart Manual extension ──────────────────────────────────
# The extension watches this file and auto-displays figures without any
# vscode:// URI prompt.
...
def _open_vscode_show_figure(payload: dict) -> bool:
    ...
    watcher picks up automatically — no vscode:// URI needed.

    Fire-and-forget: never raises — any failure here is swallowed.
    """
    try:
...
def get_figure(figure_id: str, chip_part: str) -> dict | None:
    ...
    shown = _open_vscode_show_figure(result)
    result["shown_in_vscode"] = shown
    result["note"] = (
        "Figure data retrieved successfully."
    )
    return result
